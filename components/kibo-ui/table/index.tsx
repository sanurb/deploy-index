...
export const TableHead = memo(({ header, className }: TableHeadProps) => (
  <div
    ...
    className={cn(
      ...
      "text-[11px] uppercase tracking-wide text-muted-foreground/60 font-medium",
      className,
    )}
  >
...
export interface TableHeaderProps {
  readonly className?: string
  readonly children: (props: { headerGroup: HeaderGroup<unknown> }) => ReactNode
}
...
export function TableColumnHeader<TData, TValue>({
  ...
  if (!column.getCanSort()) {
    return (
      <span className={cn("text-[11px] uppercase tracking-wide text-muted-foreground/60 font-medium", className)}>
        {title}
      </span>
    )
  }
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          className="-ml-3 h-7 text-[11px] uppercase tracking-wide text-muted-foreground/60 font-medium hover:text-muted-foreground data-[state=open]:bg-accent/50"
          size="sm"
          variant="ghost"
        >
          <span>{title}</span>
      ...
      <DropdownMenuContent align="start" className="min-w-[120px]">
        <DropdownMenuItem onClick={handleSortAsc} className="text-xs">
          <ArrowUpIcon className="mr-2 h-3 w-3 text-muted-foreground/70" />
          Ascending
        </DropdownMenuItem>
        <DropdownMenuItem onClick={handleSortDesc} className="text-xs">
...
export const TableCell = memo(({ cell, className }: TableCellProps) => (
  <div role="cell" className={cn("px-4 flex items-center min-w-0", className)}>
...
export const TableRow = memo(({ row, rowIndex, children, className }: TableRowProps) => {
  const { selectedIndex, setSelectedIndex, onRowAction, gridTemplate } = useContext(TableContext)
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "border-b dark:border-white/5 border-black/10",
        "focus:outline-none focus-visible:ring-1 focus-visible:ring-ring focus-visible:ring-inset",
        isSelected
          ? "dark:bg-white/3 bg-black/2"
          : "dark:hover:bg-white/2 hover:bg-black/2",
        className,
      )}
      style={gridStyle}
    >
...
export interface TableBodyProps {
  readonly children: (props: { row: Row<unknown>; rowIndex: number }) => ReactNode
  readonly className?: string
}
